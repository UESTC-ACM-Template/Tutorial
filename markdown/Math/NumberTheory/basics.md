...
$$
性质：$\Z_n^\times$中每一个元素的阶数都是群大小$\varphi(n)$的因子。由拉格朗日定理显然。

给定任意一个与$n$互质的$a$，若要求$a$模$n$的阶，最朴素的做法是依次计算$a$的幂。时间复杂度$O(n)$。

由上面的性质可以得到一个在给定质因数分解的前提下求元素阶数的快速做法。

设$\displaystyle ord(a)=\prod_{i=1}^kp_i^{\alpha_i},\varphi(n)=\prod_{i=1}^kp_i^{\beta_i}$，目标即计算每个$\alpha_i$。考虑到$ord(a^{p_i})=ord(a/p_i)$，因此有
$$
ord(a^{p_i^k})=ord(a)/p_i^{\min(k,\alpha_i)}
$$
因为有$\beta_i \geq \alpha_i$，所以
$$
ord(a^{p_i^{\beta_i}})=ord(a)/p_i^{\alpha_i}
$$
因此
$$
ord(a^{\varphi(n)/p_i^{\beta_i}})=p_i^{\alpha_i}
$$
