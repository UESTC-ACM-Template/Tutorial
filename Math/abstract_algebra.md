...
由于作者水平有限（主要还是菜），本章暂时与Artin第二章是基本同构的。

为了关注代数部分中与算法竞赛关联更加紧密的部分（主要还是菜），本章内的部分定理的表述会比较龙鸣，请不要在意这些细节。
...
**群中的运算可以不交换，如可逆方阵构成的乘法群和置换构成的置换群**
...
定义（子群）：$G$的任何一个满足群公理的子集$H$（即运算对$H$封闭，结合，可逆，单位元存在）被称为$G$的子群。记作$H \leq G$。
...
例4：对称群$S_3$中的子群$\{e,(12)\}$有$3$个陪集，$\{e,(12)\},\{(123),(13)\},\{(132),(23)\}$。
...
规律1：
观察到若两个元素$g_1,g_2$同属一个陪集$gH$，则$\exist h_1, h_2 \in H$使得$g_1=gh_1,g_2=gh_2$，即$g_1^{-1}g_2=h_1^{-1}g^{-1}gh_2=h_1h_2\in H$。
