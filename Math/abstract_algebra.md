...
例子中的一部分群拥有一个共同的性质，即它们中的元素都是某个单一元素的“幂”。（如整数加法群里$2$的三次“幂”不是$8$而是$6$，因为这个群的运算是加法而不是乘法）这种群被称为循环群。
...
很多群中的所有元素都能仅用其某个子集中的元素来表达。

定义（生成元）：若$G$中的每个元素都能用其子集$S$中的元素表达，则称$G$由$S$生成。

例1：$\Z$和$\Z_n$由$\{1\}$生成。

例2：$\mathbb {F}_p^\times$由模$p$意义下的原根生成。
...
定义（轨道）：给定一个$G$在$S$上的作用，可以定义一个$s$上的关系$\sim$，$s_1 \sim s_2 \Leftrightarrow \exist g \in G,g s_1 = s_2$。不难证明这是一个等价关系。这个等价关系将$G$划分为许多等价类，我们将每个等价类称为一个轨道。元素$s$所在的轨道记为$O_s$。由定义得$O_s=\{s' \in S|\exist g \in G,s'=gs \}$。
...
定义（稳定化子）：给定一个$G$在$S$上的作用，对于$S$的某个元素$t$或某个子集$T$，由$G$中满足$gt=t$或$\forall t \in T, gt \in T$的元素组成的子集（不难证明这是一个群）被称为$t/T$ 的稳定化子。
