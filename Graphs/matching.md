...
证明：因为将匹配状态反转后$P$中的任意一个点依然仅在一条匹配边中，所以得到的是一个匹配。同时因为起点和终点均未被匹配，而反转后交错路上所有点均被匹配，所以$|M'|=|M|+1$。
...
定理（增广路1）：
**对于一张图$G=(V,E)$与其上的一个匹配$M$，$M$是$G$的一个最大匹配当且仅当不存在增广路。**

证明：

充分性：已知$G$中不存在增广路，接下来需要证明$M$是最大匹配。设$G$的任意一个最大匹配为$M'$，考虑边集$E'=M \bigoplus M'$（$\bigoplus$为对称差）。
...
**对于一张图$G=(V,E)$与其上的一个匹配$M$，若$M$中不存在从$u$出发的增广路，则存在某个最大匹配$M'$使得$u$在$M'$中未被匹配**
...
若$u$在$E'$中为$1$度点，因为$M$中不存在从$u$出发的增广路，所以$u$在$E'$中的连通分量是一条长度为偶数的链，将这条链上的$M$中边与$M''$中边反转可得到一个不包含$u$的最大匹配$M'$。
